...
document, generated by `uv run python audit.py`. Top of that file is an
...
| Path | Role |
|---|---|
| `main.py`, `extract.py`, `schema.py` | House Doc PDF → structured mandates (the existing pipeline) |
| `gpo_fetch.py` | govinfo CMR catalog fetch + JSONL derivation |
| `normalize.py` | Citation parser (USC/PLAW/Stat), agency canonicalizer, text normalizer |
| `cadence.py` | "When-expected" string → cadence label + freshness window |
| `match.py` | The matcher (Stage A / B1 / B2) — outputs candidates + confident matches |
| `match_judge.py` | LLM judge harness (Claude + Gemini) for fuzzy candidates |
| `compare.py` | Consolidates judge verdicts into final outputs + writes `REPORT.md` |
| `audit.py` | Comprehensive reviewer audit — produces `AUDIT.md` |
| `judge.py`, `verify.py`, `verify_report.py` | The pre-existing extraction-accuracy harness (unrelated to the comparison flow) |
...
- **`uv run cmra`** doesn't work in this repo (the `[project.scripts]`
  ...
  Use `uv run python main.py …` instead.
